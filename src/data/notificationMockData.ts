...
const INITIAL_NOTIFICATION_MOCK_LIST: Notification[] = [
...
const getStoredNotifications = (): Notification[] => {
  if (typeof window !== 'undefined' && window.localStorage) {
    const data = localStorage.getItem('spsb_notifications')
    if (data) {
      try {
  ...
  return INITIAL_NOTIFICATION_MOCK_LIST
}

export const NOTIFICATION_MOCK_LIST: Notification[] = getStoredNotifications()

export function addNotification(notif: Omit<Notification, 'id' | 'createdAt' | 'read'>) {
  const newNotif: Notification = {
    id: `N${String(NOTIFICATION_MOCK_LIST.length + 1).padStart(3, '0')}-${Math.floor(100 + Math.random() * 900)}`,
    createdAt: new Date().toISOString().replace('T', ' ').slice(0, 16),
    read: false,
    ...notif,
  }
  NOTIFICATION_MOCK_LIST.unshift(newNotif)
  if (typeof window !== 'undefined' && window.localStorage) {
    localStorage.setItem('spsb_notifications', JSON.stringify(NOTIFICATION_MOCK_LIST))
  }
}
